...
def anim_runner(
        custom_eye_config: dict = None,
    ):

    print(f"setting up I2C")
    ...
    if custom_eye_config is not None:
        eye_config = custom_eye_config
    else:
        eye_config = {
            "left_channel": 1,
            "right_channel": 0,
    ...
    while True:
        inpt = input(">>>")
        play_anim(i2c=I2C, eye_positions=eye_config, eye="right", anim=anim_chevron_up, framesize=framesize, matrices=MATRICES, brightness=brightness, delay=delay)
        
        if inpt == "blink":
            print("blinking")
            play_anim(i2c=I2C, eye_positions=eye_config, eye="both", anim=anim_chevron_up_blink, framesize=framesize, matrices=MATRICES, brightness=brightness, delay=delay)
        elif inpt == "wink":
            play_anim(i2c=I2C, eye_positions=eye_config, eye="left", anim=anim_chevron_up_blink, framesize=framesize, matrices=MATRICES, brightness=brightness, delay=delay)
        elif inpt == "set delay":
            value = input("value:")
            delay = float(value)
        elif inpt == "set brightness":
            value = input("value:")
